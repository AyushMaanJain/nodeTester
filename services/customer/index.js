...
app.post('/add', async (req, res) => {
  const { customername, Contactno, emailid, addressLine1, addressLine2, addressLine3 } = req.body;
  
  const sql = `
    INSERT INTO Customers (customername, Contactno, emailid, addressLine1, addressLine2, addressLine3)
    VALUES (@customername, @Contactno, @emailid, @addressLine1, @addressLine2, @addressLine3);
    SELECT SCOPE_IDENTITY() AS Customerid;
  `;

  const params = [
    { name: 'customername', type: TYPES.VarChar, value: customername },
...
app.get('/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const result = await query('SELECT * FROM Customers WHERE Customerid = @id', [
...
app.put('/:id', async (req, res) => {
  const { id } = req.params;
  const { customername, Contactno, addressLine1, addressLine2, addressLine3 } = req.body;
